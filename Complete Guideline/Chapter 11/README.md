...
Center থেকে বাইরের edge পর্যন্ত distance = `0.25`

---

# 24. কেন Radius `0.25` নিলাম?

এটা কোনো fixed rule না।
...
যেকোনো suitable value নিতে পারো।

যেমন:

```cpp
float x = -0.1 + 0.5 * cos(angle);
float y =  0.0 + 0.5 * sin(angle);
```
...
# 26. শুধু X Change করলে

```cpp
float x = 0.3 + 0.25 * cos(angle);
float y = 0.0 + 0.25 * sin(angle);
```
...
# 29. `360` vs `0.25` — Confusion দূর করো

অনেক সময় মনে হতে পারে `360` আর `0.25` দুটোই Circle-এর size-related।
...
```text
360 → Shape কতটুকু ঘুরবে

0.25 → Shape কত বড় হবে
```
